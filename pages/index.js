      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .zodiac-wheel {
          ...
          width: 540px; height: 540px; opacity: 0.3;
        }
        .hero-photo {
          ...
          filter: drop-shadow(0 0 40px rgba(200,146,42,0.2));
          z-index: 2;
        }
        .pill {
        ...
        .form-section { background: #fff; border-top: 3px solid #C8922A; padding: 60px 24px; }
        ...
        .features-section { background: #FBF3E3; padding: 72px 24px; border-top: 1px solid rgba(200,146,42,0.15); }
        ...
        .section-title { font-family: 'Playfair Display', serif; font-size: clamp(24px, 3vw, 38px); font-weight: 900; color: #1a0e00; line-height: 1.2; }
        .features-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
        .feature-card { background: #fff; border: 1px solid #E8D9B8; border-radius: 12px; padding: 28px 20px; display: flex; flex-direction: column; gap: 12px; }
        .feature-card-icon {
        ...
        .stats-section { background: linear-gradient(135deg, #2a1800, #1a0e00); padding: 60px 24px; }
        .stats-grid { max-width: 900px; margin: 0 auto; display: grid; grid-template-columns: repeat(4, 1fr); gap: 32px; text-align: center; }
        .stat-val { font-family: 'Playfair Display', serif; font-size: 40px; font-weight: 900; color: #E8B84B; line-height: 1; }
        ...
        .testimonial-card { background: #FBF3E3; border: 1px solid #E8D9B8; border-radius: 12px; padding: 24px; }
        ...
        .bottom-cta { background: linear-gradient(135deg, #C8922A, #E8B84B); padding: 72px 24px; text-align: center; }
        .bottom-cta-tagline { font-family: 'Playfair Display', serif; font-size: clamp(14px,2vw,18px); color: rgba(255,255,255,0.85); font-style: italic; margin-bottom: 8px; }
        .bottom-cta-title { font-family: 'Playfair Display', serif; font-size: clamp(24px,3.5vw,44px); font-weight: 900; color: #fff; margin-bottom: 8px; }
        ...
        .footer-brand { font-family: 'Playfair Display', serif; font-size: 16px; color: rgba(255,255,255,0.6); font-weight: 700; }
        ...
        @media (max-width: 768px) {
          ...
          .hero-right { width: 100%; flex: none; height: 380px; order: -1; overflow: hidden; }
          .zodiac-wheel { width: 340px; height: 340px; }
          .hero-photo { height: 380px; max-width: 100%; }
      ...
      <section className="hero">
        <div className="hero-inner">
          <div className="hero-left">
            <div className="pill"><span>🪷</span> LEARN. UNDERSTAND. TRANSFORM.</div>
            ...
            <div className="features-row">
              {[['📖','1st Class FREE','Start Your Journey Risk-Free'],['🎓','15 Classes Complete Training','Step-by-Step From Basics to Advanced'],['📜','Certificate of Completion','Recognized Certification'],['👨‍👩‍👧‍👦','Generations of Guidance','Learn. Practice. Transform.']].map(([icon,title,sub]) => (
            ...
            <div className="trust-bar">
              {[['👥',<><strong>5000+</strong> Happy Students</>],['⭐',<><strong>15+</strong> Years Experience</>],['🌟',<><strong>4.9/5</strong> Student Rating</>],['🛡️',<><strong>100%</strong> Trusted Guidance</>]].map(([icon,text],i) => (
                <div key={i} className="trust-item"><span className="trust-icon">{icon}</span><span className="trust-text">{text}</span></div>
              ))}
            </div>
          </div>
          ...
          <div className="hero-right">
            <svg className="zodiac-wheel" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg">
              ...
              {['♈','♉','♊','♋','♌','♍','♎','♏','♐','♑','♒','♓'].map((sym,i) => {
                ...
                const x = 250+215*Math.cos(angle), y = 250+215*Math.sin(angle);
                return (
                  <g key={i}>
                    <text x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="22" fill="#C8922A">{sym}</text>
                    <line x1={250+100*Math.cos(angle)} y1={250+100*Math.sin(angle)} x2={250+195*Math.cos(angle)} y2={250+195*Math.sin(angle)} stroke="#C8922A" strokeWidth="0.5" opacity="0.4"/>
                  </g>
                );
              })}
          ...
          <div className="lead-form">
            {status === 'success' ? (
              <div className="success-box">
                ...
                <p className="success-text">Our team will call you on <strong>{phone}</strong> within 24 hours. You can also WhatsApp us directly.</p>
            ...
            {[['Ananya Mishra','Delhi','I went from knowing nothing to doing paid consultations in 6 months. Rahul sir explains in simple Hindi — even complex dashas become clear.'],['Rohit Verma','Lucknow','The practical approach is what sets this apart. I can now read full Kundali charts. The certificate helped me start my own practice.'],['Sneha Patel','Ahmedabad','Best investment I made. The live classes are incredibly valuable — Rahul sir corrects your charts in real time. Worth every rupee.']].map(([name,city,quote]) => (
              <div key={name} className="testimonial-card">
                ...
                <div className="t-author">
                  ...
                  <div><div className="t-name">{name}</div><div className="t-city">{city}</div></div>
                </div>
              </div>
            ))}
          </div>
